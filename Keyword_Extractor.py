...
class Extractor():
	...
	def printMeasures(self,table):		
		v1=[table[m1][4] for m1 in range(len(table))]
		v2=[table[m2][5] for m2 in range(len(table))]
	...
	def makeTable(self):
		...
		for skill in self.hardskills:
			if skill in self.jb_distribution:
				...
				table.append(['hard',skill,count_jb,count_cv,m1,m2])
		...
		for skill in self.softskills:
			if skill in self.jb_distribution:
				...
				m1=self.measure1(count_jb,count_cv)
				m2=self.measure2(count_jb,count_cv)
				table.append(['soft',skill,count_jb,count_cv,m1,m2])
		...
		for tuple in general_language:
			...
			if all([parts[i][1]in parts_of_speech for i in range(len(parts))]):
				...
				m1=self.measure1(count_jb,count_cv)
				m2=self.measure2(count_jb,count_cv)
				table.append(['general',skill,count_jb,count_cv,m1,m2])
		return table
